...
namespace Cortex.Server.Game.Users {
    class GameUser {
        [JsonIgnore]
        public int Id;
        ...
        public List<GameUserBadge> GetBadges() {
            ...
            using MySqlCommand command = new MySqlCommand("SELECT * FROM user_badges WHERE user = @user ORDER BY equipped_timestamp, timestamp", connection);
            command.Parameters.AddWithValue("@user", Id);
